...
def custom_loss_function(first_out, second_out, y_first, y_second):
...
def create_predicted_crosstable_hh(pred_1, pred_2, categories_1, categories_2):
    """
    Create a cross-table from predictions for household data.
    
    Args:
        pred_1, pred_2: Predicted class indices for the two attributes
        categories_1, categories_2: Category lists for the two attributes
    
    Returns:
        Dictionary representing the predicted cross-table
    """
    # Create index combinations (feature_1 feature_2)
    combinations = []
    for cat1 in categories_1:
        for cat2 in categories_2:
            combinations.append(f'{cat1} {cat2}')
    ...
    for p1, p2 in zip(pred_1_names, pred_2_names):
        combo = f'{p1} {p2}'
...
def calculate_distribution_accuracy_hh(generated_counts, target_counts):
    """
    Calculate R² accuracy comparing two distributions for household data.
    """
...
def train_model(lr, hidden_channels, num_epochs, data, targets):
    ...
    # Distribution-based accuracy function for multi-task learning
    def calculate_distribution_task_accuracy(pred_1, pred_2, target_combination, actual_crosstable):
        """
        Calculate distribution-based accuracy for a specific task combination using R².
        Uses the same restructured format as the plots for consistency.
        """
        categories_1, categories_2 = target_combination
        
        # Map attribute names to category lists
        category_map = {
        ...
        # Create predicted cross-table
        predicted_crosstable = create_predicted_crosstable_hh(pred_1, pred_2, cats_1, cats_2)
        
        # Convert actual crosstable to same format
        actual_counts = {}
        for cat1 in cats_1:
            for cat2 in cats_2:
                col_name = f'{cat1} {cat2}'
                if col_name in actual_crosstable.columns:
                    actual_counts[col_name] = actual_crosstable[col_name].iloc[0]
                else:
                    actual_counts[col_name] = 0
        
        # Calculate R² accuracy
        return calculate_distribution_accuracy_hh(predicted_crosstable, actual_counts)
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(len(targets)):
            ...
            # Calculate distribution-based accuracy only every 100 epochs to speed up training
            if (epoch + 1) % 100 == 0:
                ...
                task_accuracy = calculate_distribution_task_accuracy(
                    pred_1, pred_2, targets[i][0], actual_crosstable
                )
                epoch_task_accuracies.append(task_accuracy)

        # Calculate average accuracy for this epoch (only when accuracy is calculated)
        if epoch_task_accuracies:
    ...
    average_accuracy = sum(epoch_accuracies) / len(epoch_accuracies) if epoch_accuracies else 0.0
...
for lr in learning_rates:
    for hidden_channels in hidden_channel_options:
        ...
        results.append({
            ...
            'final_distribution_accuracy': final_accuracy,
