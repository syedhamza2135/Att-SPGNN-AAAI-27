...
"""
ACCURACY MEASUREMENT NOTE:
This script uses DISTRIBUTION-BASED ACCURACY instead of individual-level accuracy.

The model is evaluated on how well it reproduces the joint distributions from cross-tables
(e.g., sex×age×ethnicity) rather than predicting individual attributes correctly.

This approach is more appropriate because:
1. Different cross-tables assign conflicting targets to the same individuals
2. The goal is to generate a synthetic population that matches demographic distributions
3. Distribution accuracy (R²) measures how well predicted joint distributions match actual ones

Each task (sex-age-ethnicity, sex-age-marital, sex-age-religion) is evaluated separately
and the final accuracy is the average R² across all tasks.
"""
...
def create_predicted_crosstable(pred_1, pred_2, pred_3, categories_1, categories_2, categories_3):
    ...
    Returns:
        DataFrame representing the predicted cross-table
    """
...
def plotly_attribute_distributions(attribute_dicts, categories_dict, use_log=False, filter_zero_bars=False, max_cols=3):
    ...
    for idx, attr_name in enumerate(attrs):
        ...
        r2_accuracy *= 100.0  # Convert to percentage
        ...
        # Update subplot title to include R² accuracy
        fig.layout.annotations[idx].text = f"{attr_name} - Accuracy:{r2_accuracy:.2f}%"
...
def plotly_crosstable_comparison(
    ...
    for idx, crosstable_key in enumerate(keys_list):
        ...
        # Update subplot title to include R² accuracy
        fig.layout.annotations[idx].text = f"{titles[idx]} - Accuracy:{r2_accuracy:.2f}%"
