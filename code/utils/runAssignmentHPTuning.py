...
def run_command(cmd, description, working_dir=None):
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"\n[OK] Completed successfully in {duration:.1f} seconds")
            return True
        else:
            print(f"\n[ERROR] Failed with return code: {result.returncode}")
            return False
...
def main():
    ...
    for area_code in AREA_CODES:
        ...
        if missing_files:
            ...
            print(f"[WARN]  {area_code}: Missing files - {', '.join(missing_files)}")
        else:
            print(f"[OK] {area_code}: All required files found")
    
    if areas_with_missing_files:
        print(f"\n[WARN]  WARNING: {len(areas_with_missing_files)} areas have missing input files.")
    ...
    if not successful_areas:
        print(f"\n[ERROR] No areas have complete input files. Exiting.")
        return
    ...
    for area_code in AREA_CODES:
        if area_code in areas_with_missing_files:
            print(f"  {area_code}: [SKIP]  SKIPPED (missing input files)")
    ...
    if total_successful == total_processed and total_processed > 0:
        print("\n[SUCCESS] ALL PROCESSED AREAS COMPLETED SUCCESSFULLY!")
        ...
        print(f"\n[OK] {total_successful} AREAS COMPLETED SUCCESSFULLY")
        if total_failed > 0:
            print(f"[WARN]  {total_failed} AREAS FAILED - CHECK LOGS ABOVE")
        if total_skipped > 0:
            print(f"[SKIP]  {total_skipped} AREAS SKIPPED - MISSING INPUT FILES")
    else:
        print("\n[ERROR] NO AREAS COMPLETED SUCCESSFULLY")
        if total_skipped > 0:
